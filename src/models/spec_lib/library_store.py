...
class SpectrumLibraryStore:
    ...
    **Important**: ``frags`` and ``spectrum`` are stored independently.
    ...
    @classmethod
    def from_tagged(cls, target_store, tag, source_channel=None, var_tags=False):
        ...
        tag : massTag
            ...
            ``rules``, ``name``.
        source_channel: int
            Source channel, can be used in tag.mass_dict[source_channel]
        var_tags : bool
        ...
        for i in tqdm.tqdm(range(N)):
            ...
            if source_channel is not None:
                ...
                stripped_channel_peptide = peptide.replace(strip_annotation, f"({tag.name})")
                split_peptide = parse_peptide(stripped_channel_peptide)

                actual_tag_mask = np.array([res.count(f"({tag.name})") for res in split_peptide], dtype=int)
                all_tag_pos = [j for j, m in enumerate(actual_tag_mask) for _ in range(m)]
                entry_tag_pos.append(all_tag_pos)
                entry_n_tag_sites.append(len(all_tag_pos))
                entry_add_tag_masses.append(actual_tag_mask)
            else:
        ...
        for i in range(N):
            ...
            if source_channel is not None:
                src_channel_name = source_channel.replace(tag.name + "-", "")
                strip_annotation = "(" + tag.name + "-" + src_channel_name + ")"
                peptide = peptide.replace(strip_annotation, f"({tag.name})")
            ...
            for combo in combinations:
                ...
                for pos_idx, channel_idx in enumerate(combo):
                    ...
                    if source_channel is None:
                        ...
                        split_tagged[tag_pos] = split_tagged[tag_pos].replace(f"({tag.name})", f"({tag_name_c})", 1)
        ...
        # Map (source_index, combo) → output_index for resolving parent indices
        source_combo_to_out_idx = {}
        ...
        for out_idx, (i, combo, new_seq, orig_charge) in enumerate(valid):
            total_tag_mass = sum(
                float(tag.channel_masses[channel_idx]) - source_channel_mass for channel_idx in combo
            )
            ...
            source_combo_to_out_idx[(i, combo)] = out_idx

            # Resolve parent_idx: find the parent target's output index in the same combo
            old_parent = target_store.parent_idx[i] if hasattr(target_store, 'parent_idx') else -1
            if old_parent >= 0:
                out_parent_idx[out_idx] = source_combo_to_out_idx.get((old_parent, combo), -1)
