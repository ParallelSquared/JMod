...
class SpectrumFile:
    ...
    def reband_ms1_to_ms2_bands(self, im_tol):
        ...
        Must run after the preliminary search (so ``im_tol`` = the run's fitted
        ``RunState.opt_im_precision`` is known) and before the main search. No-op if MS1 lacks per-peak mobility.
        """
