...
# Note: The refactored versions of fit_to_lib, fit_to_lib_decoy, and fit_to_lib2
# are now in the spectral_fitting package. This file contains the original implementations
# which are preserved for reference but should not be used directly.
...
def get_features(
    ...
    if len(prec_frags)>0 and len(list(prec_frags)[0])==len(lib_peaks_matched[0]):
        ...
        longest_y_ions = [max([int(re.match(r"[by](\d+)",i)[1]) for i in frag_names])  for frag_names in ordered_frags]
    else:
